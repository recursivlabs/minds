  ...
  return (
    <html lang="en">
      <head>
        ...
        {/* Instant boot shell styles. The web app is a ~1MB single JS bundle, so
            without this the user stares at a blank page for ~9s (measured FCP)
            while it downloads + parses — the server itself responds in ~70ms.
            This shell paints from the served HTML immediately and is removed by
            the root layout on first paint (see app/_layout.tsx). */}
        <style
          dangerouslySetInnerHTML={{
            __html: `
#minds-boot{position:fixed;inset:0;z-index:2147483647;display:flex;flex-direction:column;
  align-items:center;justify-content:center;background:#08080a;transition:opacity .35s ease;
...
#minds-boot .mb-sp{width:34px;height:34px;border-radius:50%;border:3px solid rgba(255,255,255,.14);
  border-top-color:#f2c94c;animation:mb-spin .8s linear infinite}
@keyframes mb-spin{to{transform:rotate(360deg)}}
@media (prefers-color-scheme:light){#minds-boot{background:#fff}#minds-boot .mb-wm{color:#08080a}
  #minds-boot .mb-sp{border-color:rgba(0,0,0,.1);border-top-color:#c9962a}}
`,
          }}
        />
      ...
      <body>
        ...
        {children}
        {/* Failsafe: if the root layout never signals first paint, never leave
            the shell covering the app — fade it out after 20s regardless. */}
        <script
          dangerouslySetInnerHTML={{
